<#
.SYNOPSIS
    Backs up File System Access Control Lists (ACLs) for specified paths.

.DESCRIPTION
    This script collects and backs up ACLs (Owner, Group, DACL, SACL) for given file and folder paths.
    It supports recursive collection, provides comprehensive CLIXML backups, a CSV DACL report,
    and a detailed text log. The script is designed to be resilient, logging errors for
    individual items without halting the entire backup process.

    SACLs (Audit rules) can only be retrieved if the script is run with sufficient privileges
    (typically "Run as Administrator" and the user having "Manage auditing and security log"
    (SeSecurityPrivilege) rights). Similarly, accessing all items might require "Backup
    files and directories" (SeBackupPrivilege) rights.

.PARAMETER Path
    Specifies one or more file or folder paths for which to back up ACLs. This parameter is mandatory.

.PARAMETER Recursive
    If specified, ACLs for all child items (files and folders) within the provided paths will also be collected.

.PARAMETER OutputDirectory
    Specifies the directory where the backup outputs (CLIXML, CSV, TXT log) will be saved.
    Defaults to a subfolder named "PermissionBackups" in the script's execution directory.

.EXAMPLE
    .\Backup-FileSystemPermissions.ps1 -Path "C:\Data\ProjectA", "D:\Shared\Archive" -Recursive -Verbose
    This command backs up ACLs for "C:\Data\ProjectA" and "D:\Shared\Archive" and all their sub-items.
    Verbose output will be shown, and backups will be saved to ".\PermissionBackups".

.EXAMPLE
    .\Backup-FileSystemPermissions.ps1 -Path "C:\Confidential\report.docx" -OutputDirectory "E:\Backups\ACLs"
    This command backs up ACLs for the single file "C:\Confidential\report.docx" and saves the output
    to a timestamped subfolder within "E:\Backups\ACLs".
